...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        # 벡터스토어가 없으면 LLM만 사용하여 응답 생성 (RAG 없이)
        if not vectorstore:
            ...
            if not llm:
                raise HTTPException(
                    status_code=503,
                    detail="LLM을 초기화할 수 없습니다. 설정을 확인하세요."
                )
            ...
            try:
                ...
                print(f"⚠️ LLM 직접 호출 실패: {e}")
                # 간단한 폴백 응답
                return ChatResponse(
                    response=f"안녕하세요! 벡터스토어 연결이 없어 기본 모드로 작동 중입니다. 질문: {request.message}"
                )
